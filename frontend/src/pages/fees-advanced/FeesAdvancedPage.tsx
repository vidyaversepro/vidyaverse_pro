...
import { StatusPill, TONE } from '@/components/shared/Pill';
import {
  useConcessions,
  useInstallmentPlans,
  useDefaulters,
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4">
        ...
        action={
          <div className="flex flex-wrap gap-2">
            <Dialog open={concOpen} onOpenChange={setConcOpen}>
              <DialogTrigger asChild><Button variant="outline" className="flex-1 sm:flex-none"><Percent className="mr-2 h-4 w-4" /> Concession</Button></DialogTrigger>
              <DialogContent>
            ...
            <Dialog open={planOpen} onOpenChange={setPlanOpen}>
              <DialogTrigger asChild><Button className="flex-1 sm:flex-none"><Plus className="mr-2 h-4 w-4" /> Installment Plan</Button></DialogTrigger>
              <DialogContent>
      ...
      {defaulters && defaulters.count > 0 && (
        <Card className="rounded-2xl" style={{ borderColor: `${TONE.red}55`, background: `${TONE.red}14` }}>
          <CardContent className="flex items-center gap-3 p-4">
            <AlertTriangle className="h-5 w-5 shrink-0" style={{ color: TONE.red }} />
                ...
                {concessions.map((c) => (
                  <div key={c.id} className="flex items-center justify-between gap-3 rounded-xl border bg-card p-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">{c.type} · {c.percent ? c.percent + '%' : '₹' + c.amount} · {c.academicYear}</p>
                    </div>
                  ...
                  <div key={p.id} className="rounded-xl border bg-card p-3">
                    ...
                    <div className="mt-2 flex flex-col gap-1.5">
                      {p.installments.map((i) => (
                        <div key={i.id} className="flex flex-wrap items-center justify-between gap-1.5 text-xs">
                          <span className="min-w-0 truncate">#{i.installmentNo} · ₹{i.amount} · due {new Date(i.dueDate).toLocaleDateString('en-IN')}</span>
