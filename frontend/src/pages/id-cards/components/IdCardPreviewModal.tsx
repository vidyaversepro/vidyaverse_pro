        ...
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent aria-describedby={undefined} className="max-w-3xl max-h-[90vh] flex flex-col overflow-y-auto">
                ...
                <div className="flex-1 bg-gray-100 dark:bg-gray-800 rounded-md flex items-center justify-center p-4 overflow-hidden">
                    {/* In a real app, this would be a PDF viewer or an iframe */}
                    {idCard.pdfUrl ? (
                        <div className="text-center">
                            <div className="aspect-[1.586] w-96 bg-white shadow-lg mx-auto mb-4 flex items-center justify-center relative overflow-hidden">
                                {idCard.thumbnailUrl ? (
                                    <img src={idCard.thumbnailUrl} alt="Card Preview" className="w-full h-full object-cover" />
                                ) : (
                                    <div className="p-8 text-center text-gray-400">
                                        <p>PDF Preview</p>
                                        <p className="text-xs mt-2">{idCard.pdfUrl}</p>
                                    </div>
                                )}
                            </div>
