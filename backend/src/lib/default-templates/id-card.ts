/**
 * Curated default STUDENT ID CARD (HTML/Handlebars), CR80 landscape (85.6×54mm).
 * This is the same proven layout installed by scripts/install-idcard-template.ts
 * (the one that rendered the live cards) — kept here so templateResolver auto-seeds
 * it for every institution, exactly like the other document types.
 *
 * CONTRACT: branding flat (institutionName, institutionAddress, institutionLogo,
 *   principalSignature, principalName, principalTitle) + studentName, photoUrl(dataURI),
 *   admissionNo, className, dob, bloodGroup, fatherName, phone, academicYear,
 *   validUntil, qrCode(dataURI).
 * Fonts/reset injected by document-base.
 */
export const ID_CARD_HTML = `<style>
