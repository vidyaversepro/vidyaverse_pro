...
export const academicTabSchema = z.object({
    ...
    sectionId: z.preprocess(
        ...
        z.string().uuid("Select a valid section").optional()
    ),
    rollNo: z.preprocess(
        (val) => (val === '' || val === null || val === undefined ? undefined : val),
        z.coerce.number().int().positive("Roll number must be positive").optional()
    ),
    admissionNumber: z.string().optional().nullable(),
    dateOfAdmission: z.preprocess(
        (val) => (val === '' || val === null || val === undefined ? undefined : val),
        z.coerce.date().refine(d => d <= new Date(), "Date cannot be in the future").optional().nullable()
    ),
